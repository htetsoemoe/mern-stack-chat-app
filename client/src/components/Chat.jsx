...
const Chat = () => {
  ...
  const sendMessage = (event, file = null) => {
    ...
    if (file) {
      fetchAllMessages() // fetch all message of selected user's messages and set to messages state
    } else {
      setNewMessageText('')
                ...
                {messages.map((message, index) => (
                  ...
                  >
                    <div className={`text-left inline-block p-2 my-2 rounded-md text-sm ${message.sender === userId ? 'bg-blue-500 text-white font-semibold' : 'bg-white text-gray-500 font-semibold'}`}>
                      {message.text}
                      {message.file && (
                        <a href={`http://localhost:3500/api/uploads/${message.file}`} target='_blank' className="flex items-center gap-1 border-b">
